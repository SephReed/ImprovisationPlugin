...
function MDI_rememberRecEntryPoint() {
  recStatus.entryPage = MDI_getCurrentPage();
}
...
function MDI_createRecordingObserver(track)  {
  return function(scene, statusEngaged)  {
    if(statusEngaged == true)  {
      ...
      if (recStatus.entryPage !== null) {
        MDI_setPage(recStatus.entryPage);
        recStatus.entryPage = null;
      }
    }
  }   
}
...
function armSingleLiveTrack(trackNum)  {
  ...
  const track = MDI_liveBank[trackNum].getTrack(trackNum);
  ...
  for(var t = 0; t < LIVE_BANK_HEIGHT; t++)  {
    ...
    const targetTrack = MDI_liveBank[t].getTrack(t);
    const targetColor = targetTrack.color();
    if(t != trackNum && liveTrackArms[t] == true && colorsMatch(color, targetColor)) { 
